...
import ru.avem.resonance.communication.devices.ipp120.OwenIPP120Controller.АВТОМАТИЧЕСКИЙ_РЕЖИМ
...
class Experiment1Controller : DeviceState(), ExperimentController {
    ...
    @Volatile
    private var latrStatusEnds: String = ""
    ...
    private fun saveTestItemPoints() {
        ...
        stackTriples.forEach {
            ...
            } else if (it.first.text.toDoubleOrNull()!! > 43.0) {
    ...
    private fun startExperiment() {
        ...
        Thread {
            if (isExperimentRunning) {
                communicationModel.showStrings(АВТОМАТИЧЕСКИЙ_РЕЖИМ)
...
//                appendOneMessageToLog("Стоп/Ресет АРН")
    ...
    private fun checkLatrStatusEnds() {
        when (latrStatusEnds) {
            LATR_UP_END -> {
